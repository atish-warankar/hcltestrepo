...
resource "aws_ecr_repository" "hcltest_lambda_ecr_repo" {
...
resource "aws_ecr_lifecycle_policy" "lifecycle_policy" {
  repository = aws_ecr_repository.hcltest_lambda_ecr_repo.name
...
EOF
  depends_on = [aws_ecr_repository.hcltest_lambda_ecr_repo]
}
...
resource "null_resource" "build_push_docker_images" {
  provisioner "local-exec" {
    command = <<EOT
      # Log in to ECR
      aws ecr get-login-password --region ${data.aws_region.current.name} | docker login --username AWS --password-stdin ${aws_ecr_repository.hcltest_lambda_ecr_repo.repository_url}
      # Build Docker image
      # docker build -t lambda-docker-demo ./application/.
      docker buildx build --platform linux/amd64 -t lambda-docker-demo ./application/.
      # Tag Docker image
      docker tag lambda-docker-demo:latest ${aws_ecr_repository.hcltest_lambda_ecr_repo.repository_url}:latest
      # Push Docker image to ECR
      docker push ${aws_ecr_repository.hcltest_lambda_ecr_repo.repository_url}:latest
    EOT
  }
  
  triggers = {
    ecr_url = aws_ecr_repository.hcltest_lambda_ecr_repo.repository_url
  }
}
...
resource "aws_lambda_function" "lambda_docker" {
  ...
  package_type = "Image"
  image_uri    = "${aws_ecr_repository.hcltest_lambda_ecr_repo.repository_url}:latest"  # Update with your ECR image URL
